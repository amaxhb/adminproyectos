...
$responsable = array();
foreach ($_SESSION['proyecto']['etapa0']['planGeneralTrabajo'] as $key => $value) {
    if (strpos($key, 'E0_responsable_') !== false || strpos($key, 'E1_responsable_') !== false || strpos($key, 'E2_responsable_') !== false || strpos($key, 'E3_responsable_') !== false || strpos($key, 'E4_responsable_') !== false) {
        $responsable[] = $value;
    }
}
...
$responsablePorEtapa = array();
for ($i = 0; $i < count($etapas); $i++) {
    $responsablePorEtapa[$etapas[$i]] = array();
    foreach ($_SESSION['proyecto']['etapa0']['planGeneralTrabajo'] as $key => $value) {
        if (strpos($key, 'E' . $i . '_responsable_') !== false) {
            $responsablePorEtapa[$etapas[$i]][] = $value;
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $_SESSION['proyecto']['etapa1']['DiagramaEDT'] = $_POST;
    $_SESSION['message'] = [
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="cronogramaForm" class="form">
            ...
            <div id="DEDT">
                    ...
                    <?php foreach ($etapas as $etapa) : ?>
                            ...
                            <table class="DEDT">
                                <thead>
                                    <th colspan="2"><?php echo $etapa; ?></th>
                                </thead>
                                <tbody>
                                    <tr>
                                        <th>Actividad</th>
                                    ...
                                    <?php for ($i = 0; $i < count($actividadesPorEtapa[$etapa]); $i++) : ?>
                                        <tr>
                                            <td><?php echo $actividadesPorEtapa[$etapa][$i]; ?></td>
                                            <td><input type="text" name="<?php echo 'E' . $e. '_responsable_' . $i; ?>" value="<?php echo $responsablePorEtapa[$etapa][$i]; ?>"></td>
                                        </tr>
                                    <?php endfor; ?>
                                </tbody>
                            </table>
