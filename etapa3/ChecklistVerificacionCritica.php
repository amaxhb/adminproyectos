...
$actividadesPlaneadasCri = array();
foreach ($actividadesPlaneadas as $key => $value) {
    if ($value == 'Critica') {
        $actividadesPlaneadasCri[$key] = $value;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'] = $_POST;
    $_SESSION['message'] = [
        'text' => 'El Checklist de Verificación: Ruta Crítica se ha guardado correctamente.',
        'type' => 'success'
    ];

    header('Location: ../etapa3/ControlCambios.php');
    exit;
}
...
<body>
    ...
    <?php include_once './header3.php'; ?>
        ...
        <h2>Checklist de Verificación: Ruta Crítica</h2>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($actividadesPlaneadasCri as $key => $value) : ?>
                            <?php $key = str_replace('criticaHolgura', 'actividad', $key); ?>
                            <?php $keyCumple = str_replace('actividad', 'cumple', $key); ?>
                            <tr>
                                <td> <textarea class="input-non-editable" readonly  type="text" name="P_<?php echo $key; ?>"><?php echo $actividadesPlaneadas[$key]; ?></textarea></td>
                                <td> <textarea class="input-non-editable" readonly  type="text" name="R_<?php echo $key; ?>"><?php echo $actividadesReales[$key]; ?></textarea></td>
                                <td>
                                    <select class="select-editable" name="<?php echo $keyCumple; ?>" id="<?php echo $keyCumple; ?>">
                                        <option value="cumple" <?php if(isset($_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'][$keyCumple]) && $_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'][$keyCumple] == 'cumple') echo 'selected'; ?>>Cumple</option>
                                        <?php if($actividadesPlaneadas[$key] != $actividadesReales[$key] && !isset($_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'][$keyCumple])): ?>
                                            <option value="noCumple" selected>No Cumple</option>
                                        <?php else: ?>
                                            <option value="noCumple" <?php if(isset($_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'][$keyCumple]) && $_SESSION['proyecto']['etapa3']['ChecklistVerificacionCritica'][$keyCumple] == 'noCumple') echo 'selected'; ?>>No Cumple</option>
                                        <?php endif; ?>
                                    </select>
                                </td>
                            </tr>
